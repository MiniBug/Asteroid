...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            ...
            center = [center[0] + self.age * self.image_size[0], center[1]]
...
def click(pos):
    global started, lives, score, rock_vel_var
...
def process_sprite_group(rock_group, canvas):
    for rock in rock_group:
        rock.update()
        rock.draw(canvas)
    remove_missile = set(missile_group)
    for missile in remove_missile:
        if missile.update():
            missile_group.remove(missile)
            
def group_collide(rock_group, other_object):
    global lives, score
    remove_rock = set(rock_group)
    remove_missile = set(missile_group)
    for rock in remove_rock:
        if rock.collide(other_object):
            rock_group.remove(rock)
            expl = Sprite(rock.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
...
def rock_spawner():
    ...
    #init values for creating rock on different lpaces with different speed
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    rock_vel = [random.random() * rock_vel_var - .3, random.random() * rock_vel_var - .3]
    rock_avel = random.random() * .2 - .1
    # do not create new rock if there is 12 on screen already
